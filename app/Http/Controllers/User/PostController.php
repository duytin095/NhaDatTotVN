...
class PostController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // Retrieve properties based on purpose ID and type ID
        // $properties = Property::where('property_type_id', $typeId)
        //     ->whereHas('type', function ($query) use ($purposeId) {
        //         $query->where('property_purpose_id', $purposeId);
        //     })
        //     ->get();

        if ($typeId) {
            $type = Type::where('property_type_id', $typeId)->first();
            return $this->showByType($type->slug);
        } else {
            $purposeSlug = config('constants.property-basic-info.property-purposes')[$purposeId]['slug'];
            return $this->showByType($purposeSlug);
        }
